...
@router.post("/api/aging/adfd_model")
def execute_aging(base64_img):
    logging.info(
        "Converting base 64 string to numpy array in the adfd model router layer.")
    ...
    logging.info(
        "Checking if any face is detected in the adfd model router layer.")
    if not AgingController.check_face_alignment(numpy_array):
        return {'error': 'The face is not found.'}
    logging.info(
        "Starting to execute aging task in the adfd model router layer.")
